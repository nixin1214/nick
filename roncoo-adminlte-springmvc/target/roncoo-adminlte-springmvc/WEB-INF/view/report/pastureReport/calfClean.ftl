                        ...
                        <div class="col-md-12">
                            <div class="box box-primary">
                                <div class="box-header">
                                    <h3 class="box-title">犊牛死淘数据分析(单位:头)</h3>
                                    <div class="box-tools pull-right">
                                            ...
                                            <#list map?keys as bz>
                                                <tr>
                                                    ...
                                                    <#list ymap?keys as qj>
                                                        <#assign vmap = ymap[qj]>
                                                        <#if vmap["DNSTL-SW-TS"]==0&&vmap["DNSTL-TT-TS"]!=0>
                                                            <td>-</td>
                                                            <td>${vmap["DNSTL-TT-TS"]}</td>
                                                        <#elseif vmap["DNSTL-TT-TS"]==0&&vmap["DNSTL-SW-TS"]!=0>
                                                            <td>${vmap["DNSTL-SW-TS"]}</td>
                                                            <td>-</td>
                                                        <#elseif vmap["DNSTL-SW-TS"]==0&&vmap["DNSTL-TT-TS"]==0>
                                                            <td>-</td>
                                                            <td>-</td>
                                                        <#else >
                                                        <td>${vmap["DNSTL-SW-TS"]}</td>
...
<script type="text/javascript">
    ...
    function calfClean(yearMonth) {
         $.ajax({
               url:"${ctx}/report/pastureReport/calfCleanAjax?companyNumber=${companyNumber}&company=${company}&yearMonth="+yearMonth,
               type:"get",
               dataType:"text",
    ...
    function everydayChart(yearMonths) {
        $.ajax({
            url:"${ctx}/report/pastureReport/calfEveryAjax?companyNumber=${companyNumber}&company=${company}&yearMonths="+yearMonths,
            type:"get",
            dataType:"text",
            success: function(result){
                var jsonObj = eval('(' + result + ')');
    ...
    function th(num) {
        return $('table tr').find("td:eq(num)").html();
    }
