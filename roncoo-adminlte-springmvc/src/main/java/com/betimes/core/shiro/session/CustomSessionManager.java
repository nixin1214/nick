package com.betimes.core.shiro.session;


import com.betimes.common.utils.StringUtils;
import com.betimes.core.shiro.CustomShiroSessionDAO;
import com.roncoo.adminlte.bean.bo.UserOnlineBo;
...
public class CustomSessionManager {
	...
	public List<SimplePrincipalCollection> getSimplePrincipalCollectionByUserId(Long...userIds){
		...
		for (Session session : sessions) {
			...
			if(null != obj && obj instanceof SimplePrincipalCollection){
				...
				if(null != obj && obj instanceof RcUser){
					RcUser user = (RcUser)obj;
					//比较用户ID，符合即加入集合
	...
	private UserOnlineBo getSessionBo(Session session){
		...
		if(obj instanceof SimplePrincipalCollection){
			...
			if(null != obj && obj instanceof RcUser){
				//存储session + user 综合信息
				UserOnlineBo userBo = new UserOnlineBo((RcUser)obj);
				//最后一次和系统交互的时间
	...
	public Map<String, Object> changeSessionStatus(Boolean status,
		...
		try {
			...
			logger.error("改变Session状态错误，sessionId:{}", sessionIds, e);
			map.put("status", 500);
