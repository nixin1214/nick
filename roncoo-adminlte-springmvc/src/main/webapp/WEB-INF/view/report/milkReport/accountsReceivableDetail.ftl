                ...
                <div class="col-md-3 col-sm-6 col-xs-12">
                    <div class="info-box bg-green">
                        ...
                        <div class="info-box-content">
                            ...
                            <span class="info-box-number">${resultData[0]?round}</span>
...
<script type="text/javascript">
    ...
    $('#securityTime').datepicker({
        ...
        yearMonth = $(this).val();
        accountsReceivableDetail(yearMonth);
    ...
    function accountsReceivableDetail(yearMonth) {
        $.ajax({
            url: "${ctx}/report/milkReport/accountsReceivableDetailAjax?companyNumber=${companyNumber}&company=${company}&yearMonth="+yearMonth,
            type: "get",
            dataType: "text",
    ...
    function init(result) {
        ...
        for (var i = 0; i < result['axisx'].length; i++) {
            ...
            seriesZYYWCB.push(Math.round(result['OBC'][i]));
        }
            var option2 = {
                title: {
                    text: '近12个月走势',
    ...
    function reimbursement(yearMonths) {
        $.ajax({
            url: "${ctx}/report/milkReport/reimbursementRateAjax?companyNumber=${companyNumber}&company=${company}&yearMonths="+yearMonths,
            type: "get",
            dataType: "text",
            success: function(result){
                var jsonObj = eval('(' + result + ')');
    ...
    function queryAccountsReceivable(yearMonth) {
        $.ajax({
            url: "${ctx}/report/milkReport/queryAccountsReceivable?companyNumber=${companyNumber}&company=${company}&yearMonth="+yearMonth,
            type: "get",
            dataType: "text",
            success: function(result){
                var jsonObj = eval('(' + result + ')');
        ...
        function inti(jsonObj) {
            ...
            for(var i=0;i<5;i++){
                tbody += "<tr><td>"+(i+1)+".</td>"
                        ...
                        "</div>" +
                        "</td>" +
                        "<td style=\"text-align: center\"><span class=\"badge "+badgeColor[i]+"\">"+(data[i].growthRate==0?"-":(data[i].growthRate+"%"))+"</span></td></tr>";
            }
            $("tbody").append(tbody);
        }
    }
